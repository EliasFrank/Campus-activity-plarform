...
public class SignOff extends JFrame {
	...
	public SignOff() {
		...
		label = new JLabel("\u8BF7\u9009\u62E9\u540C\u610F\u7B7E\u9000\u7684\u4EBA");
		label.setBounds(221, 10, 268, 46);
		...
		button_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(selectedValues.length == 0) {
					new ErrorTip("提示", "请选择签退的人");
					return;
				}
				for (int i = 0; i < selectedValues.length; i++) {
					String sql = "update sign set pass=1 where id=" 
							+ getID(selectedValues[i].toString());
					ps.println(sql);//对用户进行签到
					sql = "select credit from users where account='" 
							+ getNumber(selectedValues[i].toString(), "用户账号：") + "'";
					ps.println(sql);//查询该用户的学分
					try {
						float nowCredit = Float.parseFloat(netReader.readLine());
						ps.println("select credit from activities where id=" + 
								getNumber(selectedValues[i].toString(), "活动编号："));
						float addCredit = Float.parseFloat(netReader.readLine());
		...
		JLabel label_1 = new JLabel("\u662F\u5426\u4F7F\u8FD9\u4E9B\u4EBA\u7B7E\u9000\uFF1F");
	...
	public void addList() {
		...
		try {
			...
			for (int i = 0; i < activity.length; i++) {
				ps.println("select * from sign where pass=0 and end=1 and activity=" + activity[i]);
				while(!(line = netReader.readLine()).equals("数据发送完成！！！")) {
					aList.add(line);
				}
			}
