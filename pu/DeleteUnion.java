...
public class DeleteUnion extends JFrame {
	...
	public DeleteUnion() {
		...
		button_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				for (int i = 0; i < selectedValues.length; i++) {
					String sql = "update users set isMemberOfUnion=0 where account='" 
							+ getID(selectedValues[i].toString());
					ps.println(sql);
				}
				new ErrorTip("提示", "已取消学生会成员身份！");
		...
		button_1.setBounds(468, 433, 100, 28);
		contentPane.add(button_1);
		
		JLabel label_1 = new JLabel("\u662F\u5426\u53D6\u6D88\u8FD9\u4E9B\u7528\u6237\u7684\u5B66\u751F\u4F1A\u6210\u5458\u8EAB\u4EFD\uFF1F");
		label_1.setForeground(Color.RED);
		label_1.setFont(new Font("宋体", Font.PLAIN, 20));
		label_1.setBounds(102, 432, 356, 30);
		...
		button.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				name = name + "'";
				try {
					ps.println("select * from users where isMemberOfUnion=1  and name='" + name);
					String line;
					while(!(line = netReader.readLine()).equals("数据发送完成！！！")) {
						aList.add(line);
					}
					if(aList.size() == 0) {
						new ErrorTip("提示", "查无此人，或此人不是学生会成员");
						textField.setText("");
						return;
					}
	...
	public void addList() {
		ArrayList<String> aList = new ArrayList<String>();
		try {
			ps.println("select * from users where isMemberOfUnion=1");
			String line;
			while(!(line = netReader.readLine()).equals("数据发送完成！！！")) {
				aList.add(line);
			}
